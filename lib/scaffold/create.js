...
var BASE_PACKAGE = {
  ...
  readme: 'README.md',
  dependencies: {
    'ozbcore-lib': '^' + bitcore.version,
    'ozbcore-node': version
  }
};
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/ozbcore-node.json', configJSON);
      if (!isGlobal) {
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the ozbcore-node directory and configuration
