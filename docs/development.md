...
To develop ozbcore-node:

```bash
cd ~
git clone git@github.com:<yourusername>/ozbcore-node.git
git clone git@github.com:<yourusername>/ozbcore-lib.git
```
...
We now will setup symlinks in `ozbcore-node` *(repeat this for any other modules you're planning on developing)*:
```bash
cd node_modules
rm -rf ozbcore-lib
ln -s ~/ozbcore-lib
...
cd devnode
mkdir node_modules
touch ozbcore-node.json
touch package.json
```

Edit `ozbcore-node.json` with something similar to:
```json
{
  "network": "livenet",
  "port": 3001,
...
```bash
../ozbcore-node/bin/ozbcore-node start
```